...
void rf256v2_hash_gpu(uint32_t thr_id, uint32_t threads, uint32_t startNounce, uint32_t *   output, uint64_t * __restrict__ DieRambox )
{ 
...
//	int dev_id = device_map[thr_id % MAX_GPUS];
	...
	rf256v2_hash_gpu << < gridyloop, blockyloop >> >(thr_id, threads, startNounce, d_aMinNonces[thr_id],TheRamBox[thr_id]);
